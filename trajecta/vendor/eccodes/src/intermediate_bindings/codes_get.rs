// Modified by Trajecta contributors for Windows C ABI compatibility.
...
pub unsafe fn codes_get_long_array(
    ...
    let mut key_values: Vec<c_long> = vec![0; key_size];
    ...
    Ok(key_values.into_iter().map(Into::into).collect())
}
