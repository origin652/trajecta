// Modified by Trajecta contributors for Windows support.
// See ../../TRAJECTA_PATCHES.md for the source and patch summary.
//! Definition and constructors of `CodesHandle`
//! used for accessing GRIB files

#[cfg(feature = "experimental_index")]
use crate::codes_index::CodesIndex;
use crate::{intermediate_bindings::codes_handle_new_from_file, CodesError, KeyedMessage};
#[cfg(unix)]
use crate::pointer_guard;
...
impl<D> Drop for CodesFile<D> {
    fn drop(&mut self) {
        if !self.pointer.is_null() {
            unsafe {
                libc::fclose(self.pointer);
            }
...
/// Enables ecCodes support for multiple logical GRIB fields stored in one
/// physical message on the default context.
///
/// ecCodes keeps this switch on its process-wide default context. Repeated
/// calls are harmless. Enable it before opening a GRIB file whenever logical
/// fields inside multi-field messages must be enumerated individually.
pub fn enable_grib_multi_support() {
    unsafe {
        eccodes_sys::codes_grib_multi_support_on(std::ptr::null_mut());
    }
}

impl CodesHandle<CodesFile<PathBuf>> {
    ...
    pub fn new_from_file<P: AsRef<Path>>(
        ...
        let file_pointer = open_with_fopen(&file_path)?;
...
fn open_with_fopen(path: &Path) -> Result<*mut FILE, CodesError> {
    let path = CString::new(path.to_string_lossy().as_bytes()).map_err(|_| {
        IoError::new(
            ErrorKind::InvalidInput,
            "ecCodes file path contains an interior NUL byte",
        )
    })?;
    let file_ptr = unsafe { libc::fopen(path.as_ptr(), c"rb".as_ptr()) };
...
#[cfg(windows)]
fn open_with_fmemopen(file_data: &[u8]) -> Result<*mut FILE, CodesError> {
    let file_ptr = unsafe { libc::tmpfile() };
    if file_ptr.is_null() {
        let error_val = errno();
    ...
    let written = unsafe {
        libc::fwrite(
            file_data.as_ptr().cast::<c_void>(),
            1,
            file_data.len(),
            file_ptr,
        )
    };
    if written != file_data.len() {
        unsafe {
