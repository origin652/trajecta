...
fn verify_sqlite_run_row(
    ...
    let row = connection
        .query_row(
            "SELECT run_id, manifest_schema, case_name, status FROM run",
            [],
            |row| {
                Ok((
                    row.get::<_, String>(0)?,
    ...
    if row.0 != manifest.run_id.0
        || row.1 != manifest.schema_version
        || row.2 != manifest.case_name
        || row.3 != expected_status
    {
        return Err(ResultVerificationError::new(
...
fn audit_particle_rows(connection: &Connection) -> Result<u64, ResultVerificationError> {
    let mut statement = connection
        .prepare(
            "SELECT particle_id, dry_air_mass_kg, sensitivity_weight
    ...
    while let Some(row) = rows.next().map_err(sqlite_error("read particle"))? {
        ...
        let sensitivity = row
            .get::<_, Option<f64>>(2)
            .map_err(sqlite_error("sensitivity weight"))?;
        if particle_id < 0
            || !dry_air_mass.is_finite()
            || dry_air_mass < 0.0
            || sensitivity.is_some_and(|value| !value.is_finite())
        {
            return Err(ResultVerificationError::new(
                "result.nonfinite_state",
    ...
    let invalid_masses = connection
        .query_row(
            "SELECT COUNT(*) FROM particle_mass
             WHERE NOT (mass_kg >= 0.0) OR mass_kg > 1.7976931348623157e308",
            [],
            |row| row.get::<_, u64>(0),
        )
...
mod tests {
    ...
    fn write_result(directory: &Path, status: RunLifecycleStatus) -> RunManifest {
        ...
        let mut sink = ParticleStateSqliteSink::default();
