...
fn collect_events<I>(rows: I) -> Result<Vec<StoredEvent>, JobBackendError>
where
...
struct StoredJob {
    ...
    created_seconds: i64,
    created_nanosecond: i64,
    started_seconds: Option<i64>,
    started_nanosecond: Option<i64>,
    finished_seconds: Option<i64>,
    finished_nanosecond: Option<i64>,
    output_directory: Option<String>,
}

impl StoredJob {
    fn into_snapshot(self, queue_position: Option<u64>) -> Result<JobSnapshot, JobBackendError> {
...
fn stored_job_from_row(row: &Row<'_>) -> rusqlite::Result<StoredJob> {
...
impl StoredEvent {
    fn into_event(self) -> Result<JobEvent, JobBackendError> {
        let event = JobEvent {
...
struct StoredLease {
    ...
    cancel_mode: Option<String>,
}

#[derive(Debug)]
struct StoredDaemonLease {
    daemon_instance_id: String,
    daemon_pid: i64,
...
fn stored_daemon_lease_from_row(row: &Row<'_>) -> rusqlite::Result<StoredDaemonLease> {
...
fn daemon_lease_from_stored(stored: StoredDaemonLease) -> Result<DaemonLease, JobBackendError> {
...
fn stored_lease_from_row(row: &Row<'_>, run_id: RunId) -> rusqlite::Result<StoredLease> {
    Ok(StoredLease {
        run_id,
        daemon_instance_id: row.get(0)?,
...
fn worker_lease_from_stored(stored: StoredLease) -> Result<WorkerLease, JobBackendError> {
...
const JOB_SELECT: &str = "SELECT job_series_id, run_id, attempt, state, input_json, resources_json,
