---
title: 十五分钟区域填充快速入门
description: 使用四个时次的 CFSR 演示资料，运行并完整验证一个小型区域填充水汽案例。
---

# 十五分钟区域填充快速入门

本流程使用 CFSR 全球气压层资料运行一个小型水汽追踪案例。Trajecta 在研究区域内
生成 1,000 个气团粒子，向前积分十分钟，并在起止时刻保存粒子状态。后续步骤会完成
项目校验、资料锁定、任务运行和结果读取。将案例扩大到正式研究规模时，仍沿用相同的案例、
运行配置、资料锁、任务队列和结果目录结构。

四个时次的演示资料约 22 MiB。使用已经构建好的发布包时，从解压到完成完整验证通常可在
十五分钟内完成。第一次从源码构建会花费更多时间，主要耗时来自 Cargo 编译依赖。

## 1. 获得可执行文件

可以从 [GitHub Releases](https://github.com/origin652/trajecta/releases) 下载对应平台的
发布包，也可以直接从源码构建。下面的依赖表说明纯 Rust 读取器的构建条件，以及正式
发布包携带的原生库版本。

| 构建项 | 本快速入门使用的配置 |
|---|---|
| Rust | `1.85.0`；项目最低版本为 `1.85` |
| Windows 编译工具 | 与 Rust 主机目标一致的 64 位链接器；正式原生构建使用 MSYS2 UCRT64 GNU 工具链 |
| Ubuntu 编译工具 | 与 `x86_64-unknown-linux-gnu` 匹配的 C/C++ 构建工具链 |
| ecCodes | 纯 Rust 构建无需安装；Windows 发布包为 `2.47.0`，Ubuntu 发布包为 `2.34.1` |
| netCDF-C | 纯 Rust 构建无需安装；Windows 发布包为 `4.9.3`，Ubuntu 发布包为 `4.9.2` |
| HDF5 | 纯 Rust 构建无需安装；Windows 发布包为 `1.14.6`，Ubuntu 发布包为 `1.10.10` |
...
各文件的名称、大小和 SHA-256 见[演示资料](demo-data.md)。

解压后的资料包含 2009 年 1 月 1 日 00、06、12 和 18 UTC 四个时次。示例只积分
06:00 至 06:10 这十分钟。四个时次的文件共同覆盖该数据集配置所需的时间范围。

## 3. 了解示例项目

示例位于 `examples/domain-fill-cfsr`。项目索引包含名为 `moisture` 的案例，以及名为
`product` 的运行配置。前者规定科学问题，后者补充本机路径和执行资源。

| 设置 | 本例取值 |
|---|---|
| 模拟时段 | 2009-01-01 06:00:00 至 06:10:00 UTC |
| 方向 | 向前 |
| 粒子群 | `domain_fill_air_mass`，1,000 个粒子 |
...
模拟时段、粒子群和数值设置位于 `cases/moisture.yaml`。本机路径及资源需求位于
`profiles/product.yaml`。项目索引把逻辑数据集 `cfsr` 绑定到
`cfsr-pgbl-pressure-v0`，运行时再通过资料锁确定实际使用的四个源文件。
...
`cpu_slots` 是本机可供调度器分配的 CPU 槽位。`memory_pool_mib` 设置 Trajecta 可纳入
调度的内存池，`memory_reserve_mib` 为操作系统、守护进程和同机程序预留空间。示例
运行配置申请一个工作线程和 1 GiB 内存，因此上述资源池可以接纳一个示例任务。

`quickstart.toml` 是本机配置，可以供同一台机器上的多个项目复用。案例和运行配置
仍保存在各自的项目目录中。
...
资料锁会在下一步执行 `project finalize` 时生成。

资料计划（`data-plan`）中与本例对应的需求会包含以下主要字段：
...
`partial` 表示项目文档已经解析完成，但资料锁尚未创建。下载助手读取资料计划后，
会逐一显示目标文件及其本地状态。
...
项目定稿命令 `project finalize` 读取本地文件，为
`examples/domain-fill-cfsr/locks/cfsr.lock.json`
写入文件 SHA-256、有效时次、网格签名和气压层签名，同时记录内置资料配置及其读取能力。
案例、运行配置与资料内容由此绑定为一次可运行的项目配置。

`doctor --deep` 检查所选本机配置、项目资料、资料锁和结果文件系统。深度检查还会实际创建
SQLite 数据库，写入 WAL，运行完整性检查，截断检查点并清理临时文件。命令结束时会按阶段
列出检查结果；全部通过后即可提交任务。
...
`run_id` 标识本次执行轮次，`output_directory` 给出结果目录。任务被接收后，本地守护进程
负责调度，工作进程执行数值计算并写入结果。
...
将 `RESULT` 替换为结果目录、任务系列 ID 或运行 ID。
...
完整验证成功后，结果状态为 `complete`。`result inspect` 会汇总 1,000 个粒子、
2 个输出时次、2,000 条粒子状态记录和异常终止计数。每次运行会产生新的运行 ID。

结果目录中的主要文件如下：

| 文件 | 内容 |
|---|---|
| `run-manifest.json` | 运行状态、输入、数值设置、质量统计和文件散列 |
| `particles.sqlite` | 粒子、状态、质量收支、事件和终止记录 |
| `particles.sqlite-wal` | SQLite 写前日志；成功收尾后大小为零 |
| `resolved-case.json` | 工作进程实际执行的案例文档 |
| `resolved-run-profile.json` | 本次运行实际采用的运行配置 |
| `provenance-bundle.json` | 与输出记录关联的气象来源及变换过程 |
