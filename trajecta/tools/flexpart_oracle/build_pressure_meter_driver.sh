...
BUILD_DIR="${TRAJECTA_ORACLE_BUILD_DIR:-$ROOT/target/m3-oracle/build/pressure-meter}"
...
common_flags=(
  ...
  -UUSE_NCF -UETA -Duseomp -fopenmp
  -ffunction-sections -fdata-sections
  -J"$BUILD_DIR/mod" -I"$BUILD_DIR/mod"
  -I"$eccodes_include" -I"$netcdf_include"
)

# Persist actual compile identity for oracle JSON (full flags, including -g/-U*).
...
driver_object="$BUILD_DIR/obj/pressure_meter_oracle_driver.o"
"$FC" "${common_flags[@]}" -c "$DRIVER" -o "$driver_object"

binary="$BUILD_DIR/bin/pressure_meter_oracle_driver"
...
nm -A "$binary" >"$BUILD_DIR/nm_symbols.txt" || nm "$binary" >"$BUILD_DIR/nm_symbols.txt"
{
  ...
  for sym in verttransform_gfs interpol_wind interpol_partoutput_val interpol_pbl oracle_calcpar; do
